...
# wk=xw.Book(Fpath)
# 打开选的材料表
# muban=xw.Book('cailiaowuliaoshumuban.xls')
muban = app.books.open('cailiaowuliaoshumuban.xls')
# 打开模板
f1=wk.sheets('封面')
# 读取材料表的封面
f2=muban.sheets('封面')
...
qd2.range('e19').value = bh3                                   #复制二维数据
...
bh9= qd1.range((6,16),(nrows1-1,16)).options(ndim=2).value    # 读取二维的数据
qd2.range('p19').value = bh9                                  #复制二维数据

#复制制作日期

bh10= qd1.range((6,19),(nrows1,19)).options(ndim=2).value    # 读取二维的数据
qd2.range('a19').value = bh10                                 #复制二维数据
qd2.api.Rows(nrows1+13).Delete()
#复制标签种类
...
if n==1:
    ...
    while m < n:
        ...
        sht_sample = muban.sheets['内容2']  # 要复制的sheet
        # 重点是copy
        sht_sample.api.Copy(Before=sht_sample.api)
        sht = muban.sheets['内容2 (2)']
        new_sheet_name = m
        sht.api.Name = new_sheet_name
        m+=1
    n=3
    for n2 in biaoqianming:
        sn=n2
        muban.sheets(n).api.Name=sn
        n+=1
    muban.sheets('end').delete()
    n=3
    i = 19
    for n3 in t[1:]:
        nr1 = muban.sheets(n)
        ...
        m=0
        while m< (int(n3)):
            nr1.api.Rows(f'{1+29*m}:{29+30*m}').Copy(nr1.api.Rows(f'{30+29*m}'))
            m+=1
        m=0
        while m< (int(n3)+1):
            nr1.range(f'H{28+29*m}').value=f'=清单!B{i+m}'
            m+=1
        i=i+int(n3)+1


        nr1.api.Rows(((int(n3)+1)*29+2)).Delete()
        nr1.api.Rows(((int(n3) + 1) * 29 + 1)).Delete()
